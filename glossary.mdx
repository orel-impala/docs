...
| Term | Meaning |
| --- | --- |
| **Herd** | Impala's inference engine. Profiles your live traffic and re-tunes its execution plan against it — kernels, batching, decoding. Every workload runs on it; nothing about it is yours to configure. See [Impala Herd](/impala-herd). |
| **Leap** | Live weight iteration: swap the weights a running fleet serves, in seconds, with no redeploy. Herd runs your inference; Leap changes your weights. See [Impala Leap](/weight-sync). |
...
| Term | Meaning |
| --- | --- |
| **Serverless** | Inference on Impala's cloud, no install. See [Run in our cloud, or yours](/deployment-options). |
| **BYOC** | Impala's data plane running inside your own cloud. See [Run in our cloud, or yours](/deployment-options). |
